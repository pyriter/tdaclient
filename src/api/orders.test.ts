...
describe("Orders", () => {
  ...
  it("should be able to get all orders for all linked accounts", async () => {
    let response = await getOrdersByQuery()
  ...
  it("should be able to get orders for a specific account given an account id", async () => {
    // Get account
    let accountResponse = await getAccount()
    let accountId = accountResponse[0].accountId;
    let response = await getOrdersByQuery({accountId});
  ...
  it("should be able to place a stock order and then cancel it", async () => {
    let accountResponse = await getAccount()
    let validAccount = accountResponse.filter(r => r.initialBalances.cashAvailableForTrading > 100).pop();
    let accountId = validAccount.accountId;
    let order = {
